...
try {
  foreach ($files as $f) {
    ...
    $topname = str_replace('.json', '.txt', $f);
    $topfile = "dataResults/topTen/$topname";
    $topTen = json_decode(file_get_contents($topfile));

    $tops = [];
    $all_relevant_text = array();
    foreach ($topTen as $word) {
        ...
        foreach($sec as $type=>$s) {
          foreach($s as $m=>$para) {
            if (stripos($para, $place) !== false) {
              $parawords = explode(' ', $para);
              foreach($parawords as $curr) {
                if (in_array(strtolower($curr), $tops)) {
                  array_push($all_relevant_text[$curr], $year);
                }
              }
            }
          }
    ...
    //print_r($all_relevant_text);
    $nice_place = str_replace(' ', '_', $place);
    file_put_contents( "dataResults/yearCounts/$nice_place.txt", json_encode($all_relevant_text));
