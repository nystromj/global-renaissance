...
foreach ($files as $file) {
  ...
  while (count($all_data) > 100 && count($top_ten) < 100)
  {
    if (!in_array($all_data[$x], $top_ten))
    {
	  $information = $all_data[$x];
	  array_push($information, array($categories[array_rand($categories)]));
	  array_push($top_ten, $information);
    } // if
    $x++;
	 echo $x . "<br>";
  } // while
  ...
  file_put_contents("dataResults/topWords/$file", json_encode($top_ten));
}
echo "Fin.";
